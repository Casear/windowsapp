...
function setupWorld() {
    ...
    if (navigator.msPointerEnabled) {
        ...
        document.addEventListener("MSHoldVisua"l, function (e) { e.preventDefault(); }, false);
    }
    else {
        downevent = "mousedown";
        upevent = "mouseup";
    ...
    RAINBOWSTRIP_IMAGE = document.getElementById("rainbowstrip");
...
function drawRainbows() {
    ...
    for (var i = 0; i < particles.length; i++) {
        p = particles[i];
        context.drawImage(RAINBOWSTRIP_IMAGE, RAINBOW_STRIP_DOT_WIDTH * p.hue, 0, RAINBOW_STRIP_DOT_WIDTH, RAINBOW_STRIP_DOT_WIDTH, p.x - PARTICLE_RADIUS, p.y - PARTICLE_RADIUS, PARTICLE_DIAMETER, PARTICLE_DIAMETER);
    }
}
